...
const reviews = [
  ...
  { author_name: "M. Gentile",     rating: 5, body: "Ne ho già preso un secondo paio. Pago sempre alla consegna, comodissimo come sistema. Le Bellavia sono diventate le mie scarpe quotidiane.", reply: null, days: 21 },
...
async function main() {
  const product = await sql`SELECT id FROM products WHERE slug = 'bellavia' LIMIT 1`;
  if (!product.length) { console.error("Prodotto bellavia non trovato"); process.exit(1); }
  const pid = product[0].id;

  const existing = await sql`SELECT COUNT(*) as cnt FROM reviews WHERE product_id = ${pid}`;
  if (Number(existing[0].cnt) > 0) {
    console.log(`Già ${existing[0].cnt} recensioni per bellavia, skip.`);
    process.exit(0);
  }
