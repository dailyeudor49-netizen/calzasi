...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const desc = (product?.description as string) || "Tomaia in maglia elastica, suola curva e caviglia sostenuta: un comfort che senti già dal primo passo.";
  return {
    title: product ? `${product.name} — Sandalo Estivo Comfort | ${shopName}` : `Melvia | ${shopName}`,
    description: desc,
  };
}
...
async function HeroSection() {
  ...
  const originalPrice = product ? Number(product.original_price) : 149.99;
  const discount = Math.round(((originalPrice - price) / originalPrice) * 100);
  const accentColor = (product?.color as string) || DS.brand;
  const productName = (product?.name as string) || "Melvia";
  const features = (product?.features as string[]) || [];

  /* Icons are layout (hardcoded per rules), feature text comes from DB */
  const featureIcons = [
    /* 1: tomaia elastica */ <svg key="i0" viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke={DS.brandDark} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M8 12s1.5 2 4 2 4-2 4-2"/><path d="M8 8h.01M16 8h.01"/></svg>,
        ...
        <div>
          ...
          <div style={{ marginBottom: 14 }} className="flex items-center gap-3">
            <span className="inline-flex items-center rounded px-2 py-0.5 text-[11px] font-semibold uppercase tracking-[0.08em]"
              ...
              {(product?.category_label as string) || "Sandalo Comfort Estivo"}
            </span>
            {reviewCount > 0 && (
          ...
          <h1 style={{ marginBottom: 12, color: DS.brand, fontFamily: "var(--font-heading)", fontSize: 28, lineHeight: 1.1, letterSpacing: "-0.02em" }}
            ...
            {(product?.name as string || "Melvia").toUpperCase()} — <span style={{ color: "#4A4A4A" }}>{(product?.subtitle as string) || "Si infila come un calzino."}</span>
          </h1>

          {/* Description */}
          <p style={{ marginBottom: 32, color: DS.textSec }} className="text-[16px] leading-[1.65]">
            {(product?.description as string) || "Tomaia in maglia elastica, suola curva e caviglia sostenuta: un comfort che senti già dal primo passo."}
          </p>
  ...
  const rows = [
    {
      ...
      alt: "Suola a dondolo Melvia",
      title: "Suola a Dondolo",
      desc: "La curvatura della suola accompagna il movimento e rende la camminata più fluida e naturale.",
      bg: "#FFFFFF",
    },
    {
      img: `/images/land/${SLUG}/why/2.webp`,
      alt: "Soletta in memory foam Melvia",
      title: "Soletta in Memory Foam",
      desc: "Si adatta alla forma del piede e dona una sensazione di comfort fin dal primo utilizzo.",
      bg: "#FFFFFF",
    },
    {
      img: `/images/land/${SLUG}/why/3.webp`,
      alt: "Coinvolge gambe e glutei Melvia",
      ...
      >
        <div className="mb-6 text-center md:mb-8">
          ...
          <p className="mx-auto max-w-lg text-[16px]" style={{ color: DS.textSec, lineHeight: 1.6 }}>
            Tomaia elastica, soletta in memory foam e suola curva: il comfort che cercavi ad ogni passo.
          </p>
        </div>
...
const featureCards = [
  {
    ...
    animated: true,
    alt: "Tomaia elastica Melvia",
    title: "Si adatta senza comprimere",
    badge: "Elastica e avvolgente",
    tags: ["Come un calzino"],
    text: "La tomaia elastica abbraccia il piede seguendone la forma naturale e riduce i punti di pressione.",
  },
  {
    img: `/images/land/${SLUG}/angle/2.webp`,
    alt: "Calzata immediata Melvia",
    title: "Infili e vai",
    badge: "Senza allacciature",
    tags: ["Calzata rapida"],
    text: "Niente lacci né fibbie da sistemare: basta infilare il piede. Pratici per ogni giorno.",
  },
  {
    img: `/images/land/${SLUG}/angle/3.webp`,
    alt: "Supporto caviglia Melvia",
    title: "Caviglia sostenuta",
    badge: "Appoggio stabile",
    tags: ["Sostegno elastico"],
    text: "La fascia elastica avvolge e stabilizza la caviglia, offrendo un sostegno naturale e una maggiore stabilità ad ogni passo.",
  },
  {
    img: `/images/land/${SLUG}/angle/4.webp`,
    alt: "Soletta memory foam Melvia",
    title: "Soletta in memory foam",
          ...
          <FeatureCard key={f.title} id={f.title}>
            ...
            >
              <div className="relative">
                {f.animated ? (
                  imgExists(f.img)
                    ? <img src={f.img} alt={f.alt} className="aspect-[4/3] w-full object-cover" style={{ borderRadius: "14px 14px 0 0", display: "block" }} loading="lazy" />
                    : <ImgPlaceholder label={`Foto: ${f.alt}`} className="aspect-[4/3] w-full" />
                ) : (
                  <ImgWithFallback src={f.img} alt={f.alt} label={`Foto: ${f.alt}`} width={600} height={450} className="aspect-[4/3] w-full object-cover" style={{ borderRadius: "14px 14px 0 0", display: "block" }} />
                )}
...
export default async function MelviaLanding() {
  const [product, reviews, shopConfig] = await Promise.all([
    getProductBySlug(SLUG),
    getReviewsBySlug(SLUG),
    getShopConfig(),
  ...
  const orderConfig = getOrderConfig(SLUG);
  const productName = (product?.name as string) || "Melvia";
  const productSubtitle = (product?.subtitle as string) || "Si infila come un calzino.";
  const accentColor = (product?.color as string) || DS.brand;
