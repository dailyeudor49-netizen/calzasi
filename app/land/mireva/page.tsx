...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const desc = (product?.description as string) || "Tomaia in maglia elastica, caviglia sostenuta e suola curva per un comfort che senti già dal primo passo.";
  return {
    title: product ? `${product.name} — Sandalo Estivo Comfort | ${shopName}` : `Mireva | ${shopName}`,
    description: desc,
  };
}
...
async function HeroSection() {
  ...
  const productName = (product?.name as string) || "Mireva";
      ...
      <div className="grid items-start gap-4 md:gap-6 md:grid-cols-2 lg:[grid-template-columns:1.15fr_1fr]">
        ...
        <div>
          ...
          <h1 style={{ marginBottom: 12, color: DS.brand, fontFamily: "var(--font-heading)", fontSize: 28, lineHeight: 1.1, letterSpacing: "-0.02em" }}
            ...
            {(product?.name as string || "Mireva").toUpperCase()} — <span style={{ color: "#4A4A4A" }}>{(product?.subtitle as string) || "Comodo come un calzino."}</span>
          </h1>

          {/* Description */}
          <p style={{ marginBottom: 32, color: DS.textSec }} className="text-[16px] leading-[1.65]">
            {(product?.description as string) || "Tomaia in maglia elastica, caviglia sostenuta e suola curva per un comfort che senti già dal primo passo."}
          </p>
  ...
  const rows = [
    {
      ...
      alt: "Suola a dondolo Mireva",
      title: "Suola a Dondolo",
      desc: "La curvatura della suola accompagna il passo, rendendo la camminata più fluida e naturale.",
      bg: "#FFFFFF",
    },
    {
      img: `/images/land/${SLUG}/why/2.webp`,
      alt: "Soletta in memory foam Mireva",
      title: "Soletta in Memory Foam",
      desc: "Si modella sulla forma del piede e regala una sensazione di comfort fin dal primo utilizzo.",
      bg: "#FFFFFF",
    },
    {
      img: `/images/land/${SLUG}/why/3.webp`,
      alt: "Coinvolge gambe e glutei Mireva",
      title: "Effetto Active+",
      desc: "La forma della suola favorisce il coinvolgimento di gambe e glutei mentre cammini.",
      bg: "#FFFFFF",
    },
  ];
      ...
      >
        <div className="mb-6 text-center md:mb-8">
          ...
          <p className="mx-auto max-w-lg text-[16px]" style={{ color: DS.textSec, lineHeight: 1.6 }}>
            Suola curva, soletta in memory foam e tomaia elastica: un comfort che accompagna ogni passo.
          </p>
        </div>
...
const featureCards = [
  {
    ...
    animated: true,
    alt: "Tomaia elastica Mireva",
    title: "Si adatta senza stringere",
    badge: "Elastica e avvolgente",
    tags: ["Come un calzino"],
    text: "La tomaia elastica segue la forma naturale del piede e si modella su di esso, riducendo i punti di pressione.",
  },
  {
    img: `/images/land/${SLUG}/angle/2.webp`,
    alt: "Calzata immediata Mireva",
    title: "Infili e vai",
    badge: "Senza allacciature",
    tags: ["Calzata rapida"],
    text: "Niente lacci né fibbie: basta infilare il piede. Perfetti per chi cerca praticità ogni giorno.",
  },
  {
    img: `/images/land/${SLUG}/angle/3.webp`,
    alt: "Supporto caviglia Mireva",
    title: "Caviglia sostenuta",
    badge: "Appoggio stabile",
    tags: ["Strappi in velcro"],
    text: "La struttura avvolgente con strappi in velcro sostiene la caviglia e favorisce una maggiore stabilità mentre cammini.",
  },
  {
    img: `/images/land/${SLUG}/angle/4.webp`,
    alt: "Soletta memory foam Mireva",
    title: "Soletta in memory foam",
    badge: "Comfort interno",
    tags: ["Si modella al piede"],
    text: "La soletta prende la forma del piede e accompagna ogni passo, ideale anche per i piedi più sensibili.",
  },
  {
    img: `/images/land/${SLUG}/angle/5.webp`,
    alt: "Punta ampia Mireva",
    title: "Spazio per le dita",
    badge: "Punta larga",
    tags: ["Zero compressione"],
    text: "La punta ampia lascia più spazio alle dita, per una posizione naturale che non comprime l'alluce.",
  },
  {
    img: `/images/land/${SLUG}/angle/6.webp`,
    alt: "Suola curva Mireva",
...
export default async function MirevaLanding() {
  const [product, reviews, shopConfig] = await Promise.all([
    getProductBySlug(SLUG),
    getReviewsBySlug(SLUG),
    getShopConfig(),
  ...
  const orderConfig = getOrderConfig(SLUG);
  const productName = (product?.name as string) || "Mireva";
  const productSubtitle = (product?.subtitle as string) || "Comodo come un calzino.";
  const accentColor = (product?.color as string) || DS.brand;
