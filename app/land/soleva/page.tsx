...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const desc = (product?.description as string) || "Tessuto elastico resistente che abbraccia il piede come un calzino: si estende e si adatta a ogni forma per un comfort che senti fin dal primo passo.";
  return {
    title: product ? `${product.name} — Sandalo Effetto Calzino | ${shopName}` : `Soleva | ${shopName}`,
    description: desc,
  };
}
...
async function HeroSection() {
  ...
  const productName = (product?.name as string) || "Soleva";
      ...
      <div className="grid items-start gap-4 md:gap-6 md:grid-cols-2 lg:[grid-template-columns:1.15fr_1fr]">
        ...
        <div>
          ...
          <h1 style={{ marginBottom: 12, color: DS.brand, fontFamily: "var(--font-heading)", fontSize: 28, lineHeight: 1.1, letterSpacing: "-0.02em" }}
            ...
            {(product?.name as string || "Soleva").toUpperCase()} — <span style={{ color: "#4A4A4A" }}>{(product?.subtitle as string) || "Avvolge il piede come un calzino."}</span>
          </h1>

          {/* Description */}
          <p style={{ marginBottom: 32, color: DS.textSec }} className="text-[16px] leading-[1.65]">
            {(product?.description as string) || "Tessuto elastico resistente che si adatta ad ogni forma del piede e soletta in memory foam. Come camminare sulle nuvole."}
          </p>
  ...
  const rows = [
    {
      ...
      alt: "Suola a dondolo Soleva",
      title: "Suola a Dondolo",
      desc: "La curvatura della suola guida il passo e rende la camminata più fluida e naturale.",
      bg: "#FFFFFF",
    },
    {
      img: `/images/land/${SLUG}/why/2.webp`,
      alt: "Soletta in memory foam Soleva",
      title: "Soletta in Memory Foam",
      desc: "Prende la forma del piede e regala una sensazione di comfort fin dal primo utilizzo.",
      bg: "#FFFFFF",
    },
    {
      img: `/images/land/${SLUG}/why/3.webp`,
      alt: "Coinvolge gambe e glutei Soleva",
      ...
      >
        <div className="mb-6 text-center md:mb-8">
          ...
          <p className="mx-auto max-w-lg text-[16px]" style={{ color: DS.textSec, lineHeight: 1.6 }}>
            Tessuto elastico che avvolge il piede come un calzino, soletta in memory foam e suola curva: il massimo del comfort ad ogni passo.
          </p>
        </div>
...
const featureCards = [
  {
    ...
    animated: true,
    alt: "Tessuto elastico Soleva",
    title: "Avvolge come un calzino",
    badge: "Tessuto elastico resistente",
    tags: ["Vestibilità 100%"],
    text: "Il tessuto elastico è resistente e abbraccia il piede al 100%: si estende e si adatta a ogni forma, stretta o larga, senza creare punti di pressione.",
  },
  {
    img: `/images/land/${SLUG}/angle/2.webp`,
    alt: "Calzata immediata Soleva",
    title: "Infili e vai",
    badge: "Senza allacciature",
    tags: ["Calzata rapida"],
    text: "Niente lacci né fibbie: basta infilare il piede come un calzino. Comode e pratiche ogni giorno.",
  },
  {
    img: `/images/land/${SLUG}/angle/3.webp`,
    alt: "Caviglia avvolta Soleva",
    title: "Caviglia avvolta",
    badge: "Sostegno elastico",
    tags: ["Si adatta a ogni caviglia"],
    text: "La fascia elastica avvolge e stabilizza la caviglia adattandosi alla sua forma, per una stabilità naturale mentre cammini.",
  },
  {
    img: `/images/land/${SLUG}/angle/4.webp`,
    alt: "Soletta memory foam Soleva",
    title: "Soletta in memory foam",
...
export default async function SolevaLanding() {
  const [product, reviews, shopConfig] = await Promise.all([
    getProductBySlug(SLUG),
    getReviewsBySlug(SLUG),
    getShopConfig(),
  ...
  const orderConfig = getOrderConfig(SLUG);
  const productName = (product?.name as string) || "Soleva";
  const productSubtitle = (product?.subtitle as string) || "Avvolge il piede come un calzino.";
  const accentColor = (product?.color as string) || DS.brand;
